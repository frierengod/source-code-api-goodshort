<?php
/**
 * Contoh Script Bridge API Goodshort via StreamAPI.web.id
 * ----------------------------------------------------
...
// Fungsi bantuan buat manggil endpoint Goodshort di sistem Bridge
function callGoodshortAPI($urlTujuan, $token) {
...
// UJI COBA KODE
// -----------------------------------------------------------------
$baseUrl = "https://streamapi.web.id/p/goodshort/api/v1";

// Contoh 1: Fitur Cari Film Goodshort
echo "\n=====================[ 1. Pencarian Goodshort ]==================\n";
$kataKunci = urlencode("cinta");
$halaman = 1;
$urlCari = "{$baseUrl}/search/{$kataKunci}/{$halaman}?lang=in";

$hasilCari = callGoodshortAPI($urlCari, $API_TOKEN);
if ($hasilCari) {
...
$hasilDetail = callGoodshortAPI($urlDetail, $API_TOKEN);
if ($hasilDetail) {
...
$hasilNonton = callGoodshortAPI($urlNonton, $API_TOKEN);
if ($hasilNonton) {
...
echo "▶▶▶ https://streamapi.web.id/docs/goodshort ◀◀◀\n\n";
